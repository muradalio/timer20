...
- macOS 13 или новее;
- Xcode со Swift 6 или новее;
- установленные command line tools, выбранные через `xcode-select`.

Склонируйте репозиторий и соберите приложение:
...
Если нужно вручную пересоздать иконку приложения:

```sh
swift Scripts/generate-icon.swift
```

Локальная сборка подписывается ad-hoc подписью для разработки. Для публичного распространения нужно подписать приложение Developer ID сертификатом и notarize через Apple.
